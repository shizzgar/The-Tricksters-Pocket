...
class TermuxWorkspacePathTest {
    ...
    @Test fun `prompt describes actual Termux paths jobs and skill synchronization`() {
        ...
        assertTrue(prompt.contains("termux_job_start"))
        assertTrue(prompt.contains("termux_session_*"))
        assertFalse(prompt.contains("workspace_run_background"))
        assertTrue(prompt.contains("not a confinement boundary"))
        assertTrue(prompt.contains("working_dir"))

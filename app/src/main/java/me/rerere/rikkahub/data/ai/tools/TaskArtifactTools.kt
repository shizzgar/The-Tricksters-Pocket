...
object TaskArtifactTools {
    fun create(conversationId: Uuid, assistant: Assistant, repository: WorkspaceRepository, store: TaskArtifactStore): List<Tool> {
        ...
        return listOf(Tool(
            ...
            description = "Register an existing completed file as a task result, including files produced by shell/build tools. Reads the file to record its SHA-256, workspace, source assistant and chat. Does not mark its contents as verified. Only register after its producing command succeeds. Users can open, save, share and request VerifyBro verification from the task dashboard.",

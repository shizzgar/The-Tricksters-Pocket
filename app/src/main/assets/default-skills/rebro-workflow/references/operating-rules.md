# Detailed ReBro operating reference

Adapted from the previous built-in system prompt (22 September 2026). Read only
the section needed for the current task. The current user request, live tool
schemas and enabled settings govern execution. Device inventories, paths, hashes
and validation results below are dated evidence, not observations of the current
device. Verify the relevant local facts before using them. Use the connected skills
and focused Local search together to resolve missing or changed procedures; match
official documentation to the installed version. Do not silently replace working
pins with a newer upstream release. Runtime identity and jobs belong in case state.

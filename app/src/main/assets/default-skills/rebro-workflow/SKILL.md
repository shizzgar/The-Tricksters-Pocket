---
name: rebro-workflow
description: "Вести многоэтапный Android RE case: выбрать маршрут, вызвать профильные навыки, проверить receipts и восстановиться после обрыва. Использовать для полной цепочки патч-сборка-подпись-установка-проверка."
---
...
- Вход: Нужный case, цель и scope.
- Выход: Явный маршрут и последний доказанный результат.
- Оркестратор не имеет отдельного stage workflow: он ведёт перечисленные этапы.

---
name: rebro-verify
description: "Проверять функциональный результат APK-патча после установки, выполнять baseline/target/regression checks и собирать evidence. Использовать для подтверждения поведения, не подменять его успешной подписью."
---
...
- Вход: Install receipt, acceptance tests, test target.
- Выход: verification.json и реальные evidence-файлы.
